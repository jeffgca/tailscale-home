<script lang="ts">
	...
	function applyDisconnectedError() {
		error =
			tailnet.state.connectivity?.message || 'Unable to reach Tailscale API.'
		...
		try {
			...
			if (devices.length === 0) {
				try {
					...
					if (
						servicesResult.vipServices &&
						servicesResult.vipServices.length > 0
					) {
					...
					console.debug(
						'Could not check services for auto-redirect:',
						servicesErr,
					)
				}
			}
	...
	function getOS(device: Device): string {
		if (device.distro?.name) {
			return (
				device.distro.name.charAt(0).toUpperCase() + device.distro.name.slice(1)
			)
		}
	...
	function isCurrentDevice(deviceId: string): boolean {
		return (
			tailnet.state.deviceInTailnet === true && deviceId === currentDeviceId
		)
	}

	function getSortedDevices(
		devicesArr: Device[],
		currDeviceId: string | null,
	): Device[] {
...
<div class="devices-container">
	<div class="devices-header">
		<div class="devices-title-group">
			...
			<p class="scan-meta">
				Reachability scan: {formatRelativeTime(lastReachabilityScan)}
			</p>
		</div>
		<button
			class="refresh-btn"
				...
				<div class="error-details">
					...
					{#if errorDetails.status === ConnectivityStatus.NetworkError}
						<div class="error-hint">
							This usually means you're not connected to the internet or have
							network connectivity issues.
						</div>
					{:else if errorDetails.status === ConnectivityStatus.AuthenticationFailed}
						<div class="error-hint">
							Check your API key in settings. You can create a new one in the
	...
	{:else}
		<div class="devices-list">
			{#each getSortedDevices(devices, currentDeviceId) as device (device.id)}
				<DeviceCard {device} current={isCurrentDevice(device.id)} />
			{/each}
		</div>
	{/if}
</div>
