...
import { tailnetCheckIntervalSeconds, deviceProbeIntervalSeconds } from "../lib/storage";
...
export default defineBackground(() => {
  ...
  const checkAndRunReachabilityScan = async () => {
    try {
      ...
      console.log(`Device in tailnet: ${isAvailable ? "YES" : "NO"} (IPs: ${localIPs.join(", ")})`);

      // Detect connection state changes
      const stateChanged = wasConnectedToTailnet !== null && wasConnectedToTailnet !== isAvailable;
  ...
  const startIPCheckInterval = () => {
    ...
    console.log(`Starting local IP check interval (every ${localIpCheckIntervalMs / 1000} seconds)`);
  ...
  browser.runtime.onInstalled.addListener(async () => {
    ...
    localIpCheckIntervalMs = (await tailnetCheckIntervalSeconds.getValue()) * 1000;
    ipDiscoveryIntervalMs = (await deviceProbeIntervalSeconds.getValue()) * 1000;
    console.log(`Loaded timers: tailnet check=${localIpCheckIntervalMs / 1000}s, device probe=${ipDiscoveryIntervalMs / 1000}s`);
