<script lang="ts">
	...
	import {
		...
		type TailnetContextState,
	} from '../../lib/tailnetContext'
	...
	async function checkStatus() {
		...
		if (
			previousApiConnected !== nextApiConnected ||
			previousDeviceInTailnet !== nextDeviceInTailnet
		) {
	...
	onMount(() => {
		...
		const messageListener = (message: any) => {
			if (message.type === 'DEVICE_CONNECTED_TO_TAILNET') {
				console.log(
					'Device connected to tailnet notification received, refreshing status',
				)
	...
	{:else}
		<div class="main-container">
				...
				<div class="header-content">
					...
					<div class="status-section">
						<div class="status-item">
							...
							<span
								class="status-value"
								class:status-connected={tailnetState.apiConnected === true}
								class:status-disconnected={tailnetState.apiConnected === false}
								class:status-unknown={tailnetState.apiConnected === null}
							>
						...
						<div class="status-item">
							...
							<span
								class="status-value"
								class:status-connected={tailnetState.deviceInTailnet === true}
								class:status-disconnected={tailnetState.deviceInTailnet ===
									false}
								class:status-unknown={tailnetState.deviceInTailnet === null}
							>
							...
							<button
								...
								onclick={forceCheckDeviceStatus}
								disabled={tailnetState.forceCheckingDevice}
								title="Force check if this device's IP is in the tailnet"
							>
			...
			<nav class="main-nav">
				...
				>
				<a
					href={p('/tab.html/services')}
					class:active={isActive('/tab.html/services')}>Services</a
				>
			</nav>
