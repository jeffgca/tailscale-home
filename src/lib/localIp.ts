...
export async function getLocalIPs(): Promise<string[]> {
	return new Promise((resolve) => {
		...
		if (typeof window === 'undefined') {
			console.warn(
				'getLocalIPs called in non-window context (e.g., service worker). Use discoverAndStoreLocalIPs from a page context instead.',
			)
			resolve([])
			return
		}
 ...
 */
export async function getLocalIPsWithTimeout(
	timeoutMs = 5000,
): Promise<string[]> {
	return Promise.race([
		getLocalIPs(),
 ...
 */
export async function discoverAndStoreLocalIPs(
	timeoutMs = 5000,
): Promise<string[]> {
	if (!hasWindowContext()) {
		console.warn(
			'discoverAndStoreLocalIPs called in non-window context. Local IP discovery is only available in page/content script contexts.',
		)
		return []
	}
 ...
 */
export async function getLocalIPsOrCached(
	...
	if (
		lastDiscovered &&
		now - new Date(lastDiscovered).getTime() < cacheValidityMs
	) {
...
export async function requestLocalIPDiscoveryFromPage(): Promise<string[]> {
	...
	try {
		...
		if (!errorMessage.includes('Receiving end does not exist')) {
			console.warn(
				'Could not request local IP discovery from any open page:',
				error,
			)
		}
	}
